...
static const char *bootstrap_prefix(void) {
    ...
    libroot_prefix_function get_prefix = (libroot_prefix_function)dlsym(handle, "libroot_get_jbroot_prefix");
...
static void configure_environment(const char *prefix) {
    ...
    if (!getenv("BROWSER") && join_path(candidate, prefix, "/usr/bin/uiopen") && access(candidate, X_OK) == 0)
        setenv("BROWSER", candidate, 1);
}
