...
# Select the same compiler for Cargo and nested build-script commands, even
# when a standalone system Cargo precedes rustup on PATH.
toolchain_rustc="$(rustup which --toolchain "$RUST_TOOLCHAIN" rustc)"
PATH="$(dirname "$toolchain_rustc"):$PATH"
export PATH
